...
export const getAllOfficesAndPhonesFromSpain = async () => {
    let res = await fetch("http://172.16.101.146:5444/offices?country=España")//para igualar no se le pone nada despues del ? antes del =
    let data = await res.json();
    let dataUpdate = [];
    data.forEach(val => {
        dataUpdate.push({
...
export const getOfficesByCode = async (code) => {
    let res = await fetch(`http://172.16.101.146:5444/offices?code_office=${code}`)
