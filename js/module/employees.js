export const getEmployByCode = async (code) => {
    let res = await fetch(`http://localhost:5501/employees?employee_code=${code}`);
...
export const getEmployByBossCode = async (code) => {
    let res = await fetch(`http://localhost:5501/employees?code_boss=${code}`)
...
export const getEmployeeByOfficeCode = async (code) => {
    let res = await fetch(`http://localhost:5501/employees?code_office=${code}`)
    let data = await res.json()
    return data
}
...
export const getAllNameSurnamesAndEmailInCargeOfBossSeven = async () => {
    let res = await fetch("http://localhost:5501/employees?code_boss=7")
    let data = await res.json();
    let dataUpdate = [];
    data.forEach(val => {
...
export const getAllEmployees = async () => {
    let res = await fetch("http://localhost:5501/employees?position_ne=Representante Ventas")
    let data = await res.json();
    let dataUpdate = [];
    data.forEach(val => {
        dataUpdate.push({
